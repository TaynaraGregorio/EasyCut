...
def init_db():
    ...
    try:
        ...
        if conn and conn.is_connected():
            ...
            # Migração para adicionar campos de resposta da barbearia
            if 'resposta_barbearia' not in agend_cols:
                print("[MIGRAÇÃO] Adicionando coluna 'resposta_barbearia' na tabela agendamentos...")
                cursor.execute("ALTER TABLE agendamentos ADD COLUMN resposta_barbearia TEXT")
            if 'data_resposta' not in agend_cols:
                print("[MIGRAÇÃO] Adicionando coluna 'data_resposta' na tabela agendamentos...")
                cursor.execute("ALTER TABLE agendamentos ADD COLUMN data_resposta TIMESTAMP NULL")
...
def change_cliente_password(cliente_id):
    ...
    try:
        ...
        # 1. Buscar o usuário e sua senha atual (armazenada em 'senha_hash')
        cursor.execute("SELECT senha_hash FROM clientes WHERE id = %s", (cliente_id,))
        ...
        # 2. Verificar se a senha atual fornecida corresponde à senha no banco
        if user['senha_hash'] != senha_atual:
            cursor.close()
            conn.close()
            return jsonify({'success': False, 'message': 'A senha atual está incorreta'}), 401

        # 3. Atualizar a senha no banco de dados
        cursor.execute("UPDATE clientes SET senha_hash = %s WHERE id = %s", (nova_senha, cliente_id))
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'message': 'Senha alterada com sucesso'})
    except Exception as e:
        print(f"[ERRO] Ao alterar senha do cliente {cliente_id}: {e}")
...
def get_cliente_favoritos(cliente_id):
    """Lista as barbearias favoritas de um cliente."""
    conn = get_db_connection()
    if not conn:
        return jsonify({'success': False, 'message': 'Erro de conexão'}), 500
    ...
    try:
        ...
        # A query usa um JOIN para buscar os dados da barbearia diretamente
        sql = """
            SELECT b.* 
            FROM barbearias b
            JOIN favoritos_clientes f ON b.id = f.barbearia_id
            WHERE f.cliente_id = %s
        """
        cursor.execute(sql, (cliente_id,))
        barbearias_db = cursor.fetchall()
        
        barbearias_formatadas = []
        for barbearia_db in barbearias_db:
            # Buscar serviços
            cursor.execute('SELECT nome_servico FROM servicos WHERE barbearia_id = %s', (barbearia_db['id'],))
            servicos_db = cursor.fetchall()
            ...
            formatted = {
                ...
                "services": [s['nome_servico'] for s in servicos_db] if servicos_db else [],
                "priceRange": "R$ 15 - R$ 50", # Mock
                "image": barbearia_db.get('foto_perfil') or "https://via.placeholder.com/300x200/3b82f6/ffffff?text=Barbearia"
            }
            barbearias_formatadas.append(formatted)
            
        cursor.close()
        conn.close()
        
        return jsonify({'success': True, 'favoritos': barbearias_formatadas})
...
def add_cliente_favorito(cliente_id):
    """Adiciona uma barbearia aos favoritos de um cliente."""
    data = request.get_json()
    barbearia_id = data.get('barbearia_id')
    
    if not barbearia_id:
        return jsonify({'success': False, 'message': 'barbearia_id é obrigatório'}), 400
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO favoritos_clientes (cliente_id, barbearia_id) VALUES (%s, %s)", (cliente_id, barbearia_id))
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({'success': True, 'message': 'Barbearia adicionada aos favoritos!'})
    except mysql.connector.Error as err:
        if err.errno == 1062: # Chave duplicada
            return jsonify({'success': False, 'message': 'Esta barbearia já está nos seus favoritos.'}), 409
        return jsonify({'success': False, 'message': str(err)}), 500

@app.route('/api/clientes/<int:cliente_id>/favoritos/<int:barbearia_id>', methods=['DELETE'])
def remove_cliente_favorito(cliente_id, barbearia_id):
    """Remove uma barbearia dos favoritos de um cliente."""
    conn = get_db_connection()
    if not conn: return jsonify({'success': False, 'message': 'Erro de conexão'}), 500
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM favoritos_clientes WHERE cliente_id = %s AND barbearia_id = %s", (cliente_id, barbearia_id))
        conn.commit()
        affected_rows = cursor.rowcount
        cursor.close()
        conn.close()
        if affected_rows == 0:
            return jsonify({'success': False, 'message': 'Favorito não encontrado.'}), 404
        return jsonify({'success': True, 'message': 'Barbearia removida dos favoritos.'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
def submit_avaliacao(agendamento_id):
    """Registra a avaliação do cliente para um agendamento concluído"""
    try:
        data = request.get_json()
        ...
        cursor.execute('SELECT status FROM agendamentos WHERE id = %s', (agendamento_id,))
        row = cursor.fetchone()
        ...
        cursor.execute('''
            UPDATE agendamentos 
            SET avaliacao_nota = %s, avaliacao_comentario = %s 
            WHERE id = %s
        ''', (nota, comentario, agendamento_id))
...
def list_barbearia_avaliacoes(barbearia_id):
    """Lista as avaliações recebidas pela barbearia"""
    try:
        conn = get_db_connection()
        if not conn: return jsonify({'success': False, 'message': 'Erro de conexão'}), 500
        ...
        # Buscar agendamentos que tenham nota de avaliação
        cursor.execute('''
            SELECT a.id, a.avaliacao_nota, a.avaliacao_comentario, a.data_agendamento, 
                   a.resposta_barbearia, a.data_resposta,
                   c.nome_completo as client_name, s.nome_servico as service_name
            FROM agendamentos a
        ...
        for r in rows:
            # Formatar data
            data_str = str(r['data_agendamento']) if r['data_agendamento'] else ''
            
            reviews.append({
                'id': r['id'],
                'clientName': r['client_name'],
                'clientInitial': r['client_name'][0].upper() if r['client_name'] else '?',
                'rating': float(r['avaliacao_nota']),
                'date': data_str,
                'text': r['avaliacao_comentario'] or '',
                'service': r['service_name'],
                'hasResponse': bool(r['resposta_barbearia']),
                'response': r['resposta_barbearia'],
                'responseDate': str(r['data_resposta']) if r['data_resposta'] else None
            })
...
def responder_avaliacao(agendamento_id):
    """Registra a resposta da barbearia para uma avaliação"""
    try:
        data = request.get_json()
        ...
        cursor.execute('''
            UPDATE agendamentos 
            SET resposta_barbearia = %s, data_resposta = CURRENT_TIMESTAMP
            WHERE id = %s
